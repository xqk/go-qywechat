...
import (
	"errors"
	"github.com/xqk/go-qywechat/internal/lowlevel/signature"
	"net/http"
	"net/url"
	"strconv"
)
...
func (h *LowlevelHandler) echoTestHandler(
	...
	if !signature.VerifyHTTPRequestSignature(h.token, urlObj, "") {
		rw.WriteHeader(http.StatusBadRequest)
		return
	}

	adapter := URLValuesForEchoTestAPI(urlObj.Query())
